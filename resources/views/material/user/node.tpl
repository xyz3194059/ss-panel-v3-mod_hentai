...
<script src="//cdn.staticfile.org/jquery/2.2.1/jquery.min.js"></script>
	...
	<main class="content">
		<div class="content-header ui-content-header">
			<div class="container">
				<h1 class="content-heading">節點列表</h1>
			</div>
		</div>
		<div class="container">
...
{if $node_isv6[$prefix] == 0 && $node_class[$prefix]==0}
														...
														<div data-toggle="tile" data-target="#heading{$node_order->$prefix}">
															<div class="tile-side pull-left" data-ignore="tile">
																<div class="avatar avatar-sm">
																	<span class="icon {if $node_heartbeat[$prefix]=='在線'}text-green{else}{if $node_heartbeat[$prefix]=='暫無數據'}text-green{else}text-red{/if}{/if}">{if $node_heartbeat[$prefix]=="在線"}backup{else}{if $node_heartbeat[$prefix]=='暫無數據'}toll{else}warning{/if}{/if}</span>
																</div>
															</div>
																...
																<br>
 																{if $node->node_class > $user->class}
																		...
																		<div class="card-main">
																			...
																			<p class="card-heading"><b><i class="icon icon-lg">visibility_off</i>您無查看權限,如需購買VIP請<a href="/user/shop">點擊這裏</a>。</b></p>
																			</div></div></div>
																			{else}
																	{$relay_rule = null}
																	{if $node->sort == 10}
																	...
																	<div class="card">
																		<div class="card-main">
																			...
																			<p class="card-heading" >
																				...
																				<span class="label label-brand-accent"><!--{$node->status}-->←點擊節點查看配置信息</span>
																			</p>
																	<!--{if $node->sort > 2 && $node->sort != 5 && $node->sort != 10}
																				<p>地址：<span class="label" >
																				<a href="javascript:void(0);" onClick="urlChange('{$node->id}',0,0)">請點這裏進入查看詳細信息</a>
																			{else}
																			...
																			{if $node->sort == 0||$node->sort==7||$node->sort==8||$node->sort==10}
																				...
																				{if ($node->sort==0||$node->sort==7||$node->sort==8||$node->sort==10)&&($node->node_speedlimit!=0||$user->node_speedlimit!=0)}
																					<p>節點限速：<span class="label label-green">
																	...
																	{if ($node->sort == 0 || $node->sort == 10) && $node->custom_rss == 1 && $node->mu_only != -1}
																		{foreach $node_muport as $single_muport}
																			...
																			<div class="card-main">
 																					<div class="card-inner">
 																					<p class="card-heading" >
																						<a href="javascript:void(0);" onClick="urlChange('{$node->id}',{$single_muport['server']->server},{if $relay_rule != null}{$relay_rule->id}{else}0{/if})">{$prefix} {if $relay_rule != null} - {$relay_rule->dist_node()->name}{/if} - 單 - {$single_muport['server']->server} 端口</a>
																					...
																					<p>協議：<span class="label label-brand-accent">
																						{$single_muport['user']['protocol']}
																					</span></p>

																					{if $single_muport['user']['is_multi_user'] != 0}
																					<p>協議參數：<span class="label label-green">
																						{$user->id}:{$user->passwd}
																					</span></p>
																					{/if}
																					...
																					<p>混淆參數：<span class="label label-green">
								...
								<p class="card-heading">VIP用戶節點</p>
												...
												{$id=$id+1}
													...
													<div class="tile tile-collapse">
														<div data-toggle="tile" data-target="#heading{$node_order->$prefix}">
															<div class="tile-side pull-left" data-ignore="tile">
																<div class="avatar avatar-sm">
																	<span class="icon {if $node_heartbeat[$prefix]=='在線'}text-green{else}{if $node_heartbeat[$prefix]=='暫無數據'}text-orange{else}text-red{/if}{/if}">{if $node_heartbeat[$prefix]=="在線"}backup{else}{if $node_heartbeat[$prefix]=='暫無數據'}report{else}warning{/if}{/if}</span>
																</div>
															</div>
															<div class="tile-inner">
																<div class="text-overflow">
															...
															<div class="tile-sub">
																...
																<br>
 																...
 																{if $node->node_class > $user->class}
																		...
																		<div class="card-main">
																			<div class="card-inner">
																			<p class="card-heading" align="center"><b> <i class="icon icon-lg">visibility_off</i> {$user->user_name}，您無查看VIP節點權限，如需購買VIP請<a href="/user/shop">點擊這裏</a>。</b></p>
</div></div></div>
																			{else}
																	{$relay_rule = null}
																	{if $node->sort == 10}
																		...
																		{foreach $node_muport as $single_muport}
																			...
																			<div class="card">
																				<div class="card-main">
																					<div class="card-inner">
																					<p class="card-heading" >
																						<a href="javascript:void(0);" onClick="urlChange('{$node->id}',{$single_muport['server']->server},{if $relay_rule != null}{$relay_rule->id}{else}0{/if})">{$prefix} {if $relay_rule != null} - {$relay_rule->dist_node()->name}{/if} - 單端口 Shadowsocks - {$single_muport['server']->server} 端口</a>
																						<span class="label label-brand-accent">{$node->status}</span>
																					</p>
